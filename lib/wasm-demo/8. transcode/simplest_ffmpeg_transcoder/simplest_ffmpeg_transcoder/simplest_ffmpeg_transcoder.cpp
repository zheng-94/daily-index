...
static int open_output_file(const char *filename)
{
	...
	for (i = 0; i < ifmt_ctx->nb_streams; i++) {
		...
		if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO
			...
			// 在此示例中，我们将代码转码为相同的属性（图片大小，
			// 采样率等）。 可以使用过滤器轻松更改输出流的这些属性
	...
	AVCodecContext *enc_ctx, const char *filter_spec)
{
	...
	if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO) {
		...
		// set output pixel format 设置像素输出格式
...
// filter_encode_write_frame 的局部函数
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	unsigned int i;
	const char inputFile[16] = "cuc_ieschool.ts";
	const char outputFile[17] = "cuc_ieschool.mp4";
	int got_frame;
	...
	// todo: 将packets 结构体以数据的形式输出
	// todo: 将数据拼接回packets，进行后续操作
